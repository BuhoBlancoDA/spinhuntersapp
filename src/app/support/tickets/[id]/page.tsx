...
export default async function UserTicketDetail({ params }: { params: { id: string } }) {
  ...
  // Con RLS: si no es tuyo o no existe, esto devuelve null
  ...
  return (
    <main className="relative min-h-dvh overflow-hidden">
      ...
      <div className="relative z-10 mx-auto w-full max-w-3xl px-4 py-10 md:py-14 space-y-6">
          ...
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0">
              ...
              <div className="flex items-center gap-3 mt-2 text-sm">
                ...
                <span className="text-white/60">Creado: {fmtDate(ticket.created_at)}</span>
              </div>
            </div>
            <Link
            ...
            >
              <span aria-hidden>←</span> Volver a mis tickets
            </Link>
          </div>
        ...
        {/* Responder */}
        <section className="rounded-2xl border border-white/10 bg-black/60 backdrop-blur-md p-6 sm:p-8">
          <h2 className="text-lg font-semibold mb-4">Añadir respuesta</h2>
