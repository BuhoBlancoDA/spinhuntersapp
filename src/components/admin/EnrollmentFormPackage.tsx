...
export default function EnrollmentFormPackage({ packageId }:{ packageId:number }) {
  ...
  async function onEnroll(e: React.FormEvent) {
    ...
    if (!confirm(`¿Confirmas inscribir a ${label} en este paquete (${durLabel}) con estado ${status}?`)) {
      return
    }
    ...
    const res = await fetch('/api/admin/course-packages/enroll', {
      method:'POST',
    ...
    setMsg('Inscripción al paquete guardada.')
